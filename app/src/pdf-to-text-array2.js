...
function pdfToTextArray(pdfUrl) {
   ...
   */
   return Promise.all(pages.map(function(pageNumber) {
           ...
           return page.getTextContent().then(function(textContent) {
             var lastOffset = 0;
               return textContent.items.map(function(item) {
                  var xPos = item.transform[4];
                  var yPos = item.transform[5];
                  textMap[yPos].push({
                    xPos: xPos,
                    text: item.str,
                  });
    ...
    // Sort Y positions to list
    var yPosList = Object.keys(textMap).sort(function(a, b) {
      return a - b;
    });

    yPosList.forEach(function(yPos) {
      sortedTextArray.push(
        textMap[yPos].sort(function(a, b) {
