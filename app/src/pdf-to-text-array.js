...
function pdfToTextArray(pdfUrl) {
  return PDFJS.getDocument(pdfUrl).promise.then(function(pdf) {
    ...
    /*
     * This is a bit poorly documented but basically each section of text on the
     * PDF is represented by a six element transform array that PDF.js uses to
     * overlay the text content back onto the rendering of the PDF document.
     *
     * I simply use the vertical offset of the text item (transform[5]) to
     * determine where a new "line" is in the PDF document. It's a bit hacky but
